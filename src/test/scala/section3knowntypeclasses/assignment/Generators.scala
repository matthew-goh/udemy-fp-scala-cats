...
trait Generators {
  ...
  implicit def arbGet[A](implicit arbA: Arbitrary[A]): Arbitrary[Get[A]] = {
    // if successful, drop a random % of the bytes and
    val successful: Gen[Get[A]] =
      for {
        pctToDrop <- Gen.choose(0, 100)
        a         <- arbA.arbitrary // extract the a from Gen[A]
      } yield Get(bytes => Right((bytes.drop(bytes.length * pctToDrop / 100), a)))

    // generate a random string, then create a Get that returns a Left with that string
