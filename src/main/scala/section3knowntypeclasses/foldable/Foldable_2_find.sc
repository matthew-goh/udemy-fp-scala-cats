...
// find() looks inside the foldable and tries to find an element that satisfies the predicate
// alternatively, use an implicit parameter for the Foldable
def find[F[_]: Foldable, A](fa: F[A])(p: A => Boolean): Option[A] =
  fa.foldLeft(None: Option[A])((b, a) => if(p(a)) Some(a) else b)

find(MList[Int](1,2,3))(_ % 2 == 0)
find(MList[Int](1,3))(_ % 2 == 0)
