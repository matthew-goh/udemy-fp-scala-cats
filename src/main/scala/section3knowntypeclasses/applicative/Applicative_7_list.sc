...
val listApplicative: Applicative[List] = new Applicative[List] {
  ...
  override def ap[A, B](ff: List[A => B])(fa: List[A]): List[B] =
    (ff, fa) match {
      case (f :: fs, a :: as) => (a :: as).fmap(f) ++ ap(fs)(a :: as) // use the map from cats
      case _ => Nil // either list is empty
    }
}

listApplicative.map2(List(1,2,3), List(4,5))(_ + _) // take the sum of each pair and collect all results in a list
